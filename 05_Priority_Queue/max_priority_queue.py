...
def heapify(pq, i):
    if pq.left_child(i) < pq.heapsize and pq.heap[pq.left_child(i)] > pq.heap[i]:
        maximum = pq.left_child(i)
    else:
        maximum = i
    if pq.right_child(i) < pq.heapsize and pq.heap[pq.right_child(i)] > pq.heap[maximum]:
        maximum = pq.right_child(i)
    if maximum != i:
        pq.heap[i], pq.heap[maximum] = pq.heap[maximum], pq.heap[i]
        heapify(pq, maximum)
...
# Higher value = "more important" (higher up in the priority queue)
